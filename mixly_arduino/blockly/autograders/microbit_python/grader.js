/*
Overrides for generic Grader code generation.
*/
'use strict';

goog.provide('Blockly.Grader');

goog.require('Blockly.Generator');


Blockly.Grader = new Blockly.Generator('Grader');
Blockly.Grader.ORDER_ATOMIC = 0; // 0 "" ...
Blockly.Grader.ORDER_UNARY_POSTFIX = 1; // expr++ expr-- () [] .
Blockly.Grader.ORDER_UNARY_PREFIX = 2; // -expr !expr ~expr ++expr --expr
Blockly.Grader.ORDER_MULTIPLICATIVE = 3; // * / % ~/
Blockly.Grader.ORDER_ADDITIVE = 4; // + -
Blockly.Grader.ORDER_SHIFT = 5; // << >>
Blockly.Grader.ORDER_RELATIONAL = 6; // is is! >= > <= <
Blockly.Grader.ORDER_EQUALITY = 7; // == != === !==
Blockly.Grader.ORDER_BITWISE_AND = 8; // &
Blockly.Grader.ORDER_BITWISE_XOR = 9; // ^
Blockly.Grader.ORDER_BITWISE_OR = 10; // |
Blockly.Grader.ORDER_LOGICAL_AND = 11; // &&
Blockly.Grader.ORDER_LOGICAL_OR = 12; // ||
Blockly.Grader.ORDER_CONDITIONAL = 13; // expr ? expr : expr
Blockly.Grader.ORDER_ASSIGNMENT = 14; // = *= /= ~/= %= += -= <<= >>= &= ^= |=
Blockly.Grader.ORDER_NONE = 99; // (...)

Blockly.Grader.init = function(workspace) {
  /**
    * Empty loops or conditionals are not allowed in Grader.
    */
  Blockly.Grader.PASS = this.INDENT + 'pass\n';
  // Create a dictionary of definitions to be printed before the code.
  Blockly.Grader.definitions_ = Object.create(null);
  // Create a dictionary mapping desired function names in definitions_
  // to actual function names (to avoid collisions with user functions).
  Blockly.Grader.functionNames_ = Object.create(null);
  Blockly.Grader.setups_ = Object.create(null);

  if (!Blockly.Grader.variableDB_) {
    Blockly.Grader.variableDB_ =
        new Blockly.Names(Blockly.Grader.RESERVED_WORDS_);
  } else {
    Blockly.Grader.variableDB_.reset();
  }
}

Blockly.Grader.finish = function(code) {
    ...
    for (var name in Blockly.Grader.setups_) {
      setups.push(Blockly.Grader.setups_[name]);
    }
    if(setups.length !== 0)
      setups.push('\n');
    // Clean up temporary data.
    //delete Blockly.Grader.definitions_;
    //delete Blockly.Grader.functionNames_;
    //Blockly.Grader.variableDB_.reset();
    if(code !== "") 
 ...
 */
Blockly.Grader.scrubNakedValue = function(line) {
    return line + '\n';
};

/**
 * Encode a string as a properly escaped Grader string, complete with quotes.
 * @param {string} string Text to encode.
 * @return {string} Grader string.
 * @private
 */
Blockly.Grader.quote_ = function(string) {
    return "\"" + string + "\"";
};

/**
 * Common tasks for generating Grader from blocks.
 * Handles comments for the specified block and any connected value blocks.
 * Calls any statements following this block.
 * @param {!Blockly.Block} block The current block.
 * @param {string} code The Grader code created for this block.
 * @return {string} Grader code with comments and subsequent blocks added.
 * @private
 */
Blockly.Grader.scrub_ = function(block, code) {
    var commentCode = '';
    // Only collect comments for blocks that aren't inline.
    if (!block.outputConnection || !block.outputConnection.targetConnection) {
        // Collect comment for this block.
        var comment = block.getCommentText();
        comment = Blockly.utils.wrap(comment, Blockly.Grader.COMMENT_WRAP - 3);
        if (comment) {
            if (block.getProcedureDef) {
                // Use a comment block for function comments.
                commentCode += '"""' + comment + '\n"""\n';
            } else {
                commentCode += Blockly.Grader.prefixLines(comment + '\n', '# ');
            }
        }
        ...
        for (var i = 0; i < block.inputList.length; i++) {
            if (block.inputList[i].type == Blockly.INPUT_VALUE) {
                ...
                if (childBlock) {
                    var comment = Blockly.Grader.allNestedComments(childBlock);
                    if (comment) {
                        commentCode += Blockly.Grader.prefixLines(comment, '# ');
                    }
                }
    ...
    var nextCode = Blockly.Grader.blockToCode(nextBlock);
 ...
 */
Blockly.Grader.getAdjustedInt = function(block, atId, opt_delta, opt_negate) {
    var delta = opt_delta || 0;
    if (block.workspace.options.oneBasedIndex) {
        /* delta--;*/   //Keep in line with Grader
    }
    var defaultAtIndex = block.workspace.options.oneBasedIndex ? '1' : '0';
    var atOrder = delta ? Blockly.Grader.ORDER_ADDITIVE :
        Blockly.Grader.ORDER_NONE;
    var at = Blockly.Grader.valueToCode(block, atId, atOrder) || defaultAtIndex;
...
Blockly.Grader.CLASS_LCD1602_INIT = 'class LCD1602():\n'+
...
Blockly.Grader.CLASS_OLED12864_I2C_INIT='class OLED12864_I2C():\n'+
                                        '    def __init__(self):\n'+
                                        '        cmd = [\n'+
...
Blockly.Grader.CLASS_DS1307_INIT =  'DS1307_I2C_ADDRESS  = (104)\n'+
                                    'DS1307_REG_SECOND   = (0)\n'+
                                    'DS1307_REG_MINUTE   = (1)\n'+
                                    'DS1307_REG_HOUR     = (2)\n'+
                                    ...
                                    '\n'+
                                    'ds = DS1307()\n'
Blockly.Grader.FUNCTION_MIXLY_RGB_SHOW = 'def mixly_rgb_show(led, r, g, b):\n' +
                                         '    np[led-1] = (r, g, b)\n' +
                                         '    np.show()\n';
