...
Blockly.Events.fire = function (event) {
  ...
  var recordLine = {};
  recordLine.actionType = event.type;
  recordLine.oldValue = event.oldValue,
  recordLine.newValue = event.newValue,
  recordLine.time = new Date();
  var currentCode = Blockly.Python ? Blockly.Python.workspaceToCode(Blockly.mainWorkspace): Blockly.Arduino.workspaceToCode(Blockly.mainWorkspace);
  if(event instanceof Blockly.Events.Ui){ // event object is a block
    recordLine.currentCode = currentCode;
    window.actionArrayRecord.push(recordLine);
  }
  else if(event.workspaceId == Blockly.mainWorkspace.id){
    if(!(event instanceof Blockly.Events.Move && currentCode === Blockly.lastCode)){
      Blockly.lastCode = currentCode;
      var thisBlock = Blockly.mainWorkspace.getBlockById(event.blockId);
      recordLine.blockId = thisBlock.id;
      recordLine.blockName - thisBlock.type;
      recordLine.newparentBlock = thisBlock.parentBlock_? thisBlock.parentBlock_.id: undefined;
      recordLine.currentCode = currentCode;
      var childOfThisBlock = [];
      for (var each of thisBlock.childBlocks_){
        childOfThisBlock.push(each.id);
      }
      recordLine.newchildBlocks = childOfThisBlock;
      window.actionArrayRecord.push(recordLine);
    }
  }
};
