var $builtinmodule = function (name) {
    ...
    function has_proper_peer() {
        if(sm.radio.peer.channel != mod.data.channel 
            || sm.radio.peer.group != mod.data.group 
            || sm.radio.peer.address != mod.data.address
            || sm.radio.peer.data_rate != mod.data.data_rate) {
            return false;
        }     
        return true;
    }
    ...
    mod.receive_bytes = new Sk.builtin.func(function() {
        ...
        if(sm.radio.data.buffer.length > 0) {
            var data = sm.radio.data.buffer[0];
            sm.radio.data.buffer = sm.radio.data.buffer.slice(1);
            var bytes = [];
