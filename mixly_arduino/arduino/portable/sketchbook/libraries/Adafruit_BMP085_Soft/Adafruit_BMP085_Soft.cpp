...
boolean Adafruit_BMP085_Soft::begin(uint8_t mode, SoftwareWire *theWire) {
...
uint8_t Adafruit_BMP085_Soft::read8(uint8_t a) {
  ...
  _wire->beginTransmission(BMP085_I2CADDR); // start transmission to device 
#if (ARDUINO >= 100)
  _wire->write(a); // sends register address to read from
#else
  _wire->send(a); // sends register address to read from
#endif
  _wire->endTransmission(); // end transmission
  
  _wire->beginTransmission(BMP085_I2CADDR); // start transmission to device 
  _wire->requestFrom(BMP085_I2CADDR, 1);// send data n-bytes read
#if (ARDUINO >= 100)
  ret = _wire->read(); // receive DATA
#else
  ret = _wire->receive(); // receive DATA
#endif
  _wire->endTransmission(); // end transmission
...
uint16_t Adafruit_BMP085_Soft::read16(uint8_t a) {
  ...
  _wire->beginTransmission(BMP085_I2CADDR); // start transmission to device 
  _wire->requestFrom(BMP085_I2CADDR, 2);// send data n-bytes read
#if (ARDUINO >= 100)
  ret = _wire->read(); // receive DATA
  ret <<= 8;
  ret |= _wire->read(); // receive DATA
...
#if (ARDUINO >= 100)
  _wire->write(a); // sends register address to read from
  _wire->write(d);  // write data
#else
  _wire->send(a); // sends register address to read from
  _wire->send(d);  // write data
#endif
  _wire->endTransmission(); // end transmission
}
