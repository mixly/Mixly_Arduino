/*!
 *  @file Adafruit_TCS34725_Soft.cpp
 *
 ...
 *  @param  value
 */
void Adafruit_TCS34725_Soft::write8(uint8_t reg, uint32_t value) {
 ...
 *  @return value
 */
uint8_t Adafruit_TCS34725_Soft::read8(uint8_t reg) {
  _wire->beginTransmission(_i2caddr);
#if ARDUINO >= 100
  _wire->write(TCS34725_COMMAND_BIT | reg);
#else
 ...
 *  @return value
 */
uint16_t Adafruit_TCS34725_Soft::read16(uint8_t reg) {
  uint16_t x;
  uint16_t t;
 ...
 */
void Adafruit_TCS34725_Soft::enable() {
 ...
 */
void Adafruit_TCS34725_Soft::disable() {
 ...
 *          Gain
 */
Adafruit_TCS34725_Soft::Adafruit_TCS34725_Soft(tcs34725IntegrationTime_t it,
                                     tcs34725Gain_t gain) {
 ...
 */
boolean Adafruit_TCS34725_Soft::begin(uint8_t addr, SoftwareWire *theWire) {
  _i2caddr = addr;
  _wire = theWire;
 ...
 *  @return True if initialization was successful, otherwise false.
 */
boolean Adafruit_TCS34725_Soft::init() {
  _wire->begin();
 ...
 *          Integration Time
 */
void Adafruit_TCS34725_Soft::setIntegrationTime(tcs34725IntegrationTime_t it) {
 ...
 */
void Adafruit_TCS34725_Soft::setGain(tcs34725Gain_t gain) {
  /* Update the timing register */
 ...
 *          Clear channel value
 */
void Adafruit_TCS34725_Soft::getRawData(uint16_t *r, uint16_t *g, uint16_t *b,
                                   uint16_t *c) {
 ...
 *          Clear channel value
 */
void Adafruit_TCS34725_Soft::getRawDataOneShot(uint16_t *r, uint16_t *g, uint16_t *b,
                                          uint16_t *c) {
  enable();
  getRawData(r, g, b, c);
  disable();
 ...
 */
void Adafruit_TCS34725_Soft::getRGB(float *r, float *g, float *b) {
 ...
 */
uint16_t Adafruit_TCS34725_Soft::calculateColorTemperature(uint16_t r, uint16_t g,
                                                      uint16_t b) {
 ...
 *  @return Color temperature in degrees Kelvin
 */
uint16_t Adafruit_TCS34725_Soft::calculateColorTemperature_dn40(uint16_t r,
                                                           uint16_t g,
                                                           uint16_t b,
                                                           uint16_t c) {
 ...
 *  @return Lux value
 */
uint16_t Adafruit_TCS34725_Soft::calculateLux(uint16_t r, uint16_t g, uint16_t b) {
  float illuminance;
 ...
 */
void Adafruit_TCS34725_Soft::setInterrupt(boolean i) {
 ...
 */
void Adafruit_TCS34725_Soft::clearInterrupt() {
  _wire->beginTransmission(_i2caddr);
#if ARDUINO >= 100
 ...
 *          High limit
 */
void Adafruit_TCS34725_Soft::setIntLimits(uint16_t low, uint16_t high) {
